...
import { ExtendedHass, ExtendedHassEntity, ListItems, mediaCardAlbumData } from "../const/types.js";
...
export class MassBrowserAlbumView extends LitElement {
  // See setter
  private _albumData!: mediaCardAlbumData;
  ...
  // Data about album - URI, name, thumbnail, etc.
  @property({ attribute: false })
  public set albumData(data: mediaCardAlbumData) {
    this._albumData = data;
    void this.getTracks();
  }
  public get albumData() {
  ...
  public async getTracks() {
    ...
    const tracks = await this.browserActions.actionGetAlbumTracks(this.albumData.media_content_id, this.activePlayer.entity_id);
  ...
  public async getMetadata() {
    ...
    const metadata = await this.browserActions.actionGetAlbumData(this.albumData.media_content_id, this.activePlayer.entity_id);
  ...
  private onEnqueue = (ev: MenuButtonEventData) => {
    ...
    this.onEnqueueAction(this.albumData, value);
  };
  ...
  protected renderImage(): TemplateResult {
    const img = this.albumData.media_image;
    return html`
        <img
          src="${img}"
          id="img-header"
  ...
  protected renderOverview(): TemplateResult {
    ...
    const artistStr = artistLs.join(', ')
    return html`
      <div id="title">
        <ha-marquee-text>
          ${this.albumData.media_title}
        </ha-marquee-text>
      </div>
  ...
  protected renderTrack(track: Track, divider: boolean) {
    return cache(html`
      <mpc-track-row
        .track=${track}
        ?divider=${divider}
        .collectionURI=${this.albumData.media_content_id}
