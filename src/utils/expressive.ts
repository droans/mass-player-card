...
export async function applyExpressiveSchemeFromImage(
  ...
  const schemeResult = await generateExpressiveSchemeFromImage(img, hass, scheme, defaultImage);
...
async function generateExpressiveSourceColorForImageElement(elem: HTMLImageElement) {
  return await sourceColorFromImage(elem);
}

export async function generateExpressiveSchemeFromImageElement(elem: HTMLImageElement, scheme: ExpressiveScheme, darkMode: boolean): Promise<DynamicScheme> {
...
function _generateExpressiveSchemeFromColor(source: number, scheme: ExpressiveScheme, darkMode: boolean): DynamicScheme {
...
function generateExpressiveSchemeFromColor(source: number, scheme: ExpressiveScheme, darkMode: boolean) {
  return _generateExpressiveSchemeFromColor(source, scheme, darkMode)
}

export function applyExpressiveScheme(scheme: DynamicScheme, elem: HTMLElement) {
